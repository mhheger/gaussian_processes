...
find_mode_laplace <- function(K, y, likelihood_fun){
  ...
  response_fct <- function(f){
    res <- -response_list[[likelihood_fun]]$log_p(y,f) +0.5*t(f)%*%solve(K+diag(10^(-10),nrow = nrow(K)),f)+0.5*(log(abs(det(K+diag(10^(-10),nrow = nrow(K)))))+length(y)*log(2*pi))
    attr(res,"gradient") <- -1*(response_list[[likelihood_fun]]$del_f(y,f)- solve(K+diag(10^(-10),nrow = nrow(K)),f))
    attr(res,"hessian") <- -1*(response_list[[likelihood_fun]]$hess_f(y,f)- solve(K+diag(10^(-10),nrow = nrow(K))))
    return(res)
  }
  f<- rep(0,length = length(y))
  f <- nlm(response_fct,f,check.analyticals=F)$estimate
  tryCatch(
    log_q <- -0.5*t(f)%*%solve(K+diag(10^(-10),nrow = nrow(K)),f)+response_list[[likelihood_fun]]$log_p(y,f)-
      0.5*log(det(K+diag(10^(-10),nrow = nrow(K)))*det(solve(K+diag(10^(-10),nrow = nrow(K)))+response_list[[likelihood_fun]]$hess_f(y,f)))
  )
