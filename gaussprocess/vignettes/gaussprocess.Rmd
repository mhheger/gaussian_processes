...
save all the informations of class label about covariance functions, parameters 
and data in seperate objects. 
</i>

In the following example, we initialize a new object assuming 3 different 
class labels and using squared exponential for the first class, 
gamma-exponential for the second class. 
```{r}
p_class <- gp_classification$new(n=3, c("squared_exp", "gamma_exp", "squared_exp"))
```

##### Adding data 
After initalizing a new instance, we want to train the model using training 
...
We will create 30 data points uniformly distributed in the 2x6 square 
and create the vector y. 

```{r}
train_data <- tibble (x1 = runif(n=30, 0, 6),
                      x2 = runif(n=30, 0, 2))
...
paramters should be modified, this argument is necessary.
...
```{r}
p_class$set_parameters(2, cov_name = "rational", parameter_list = list(alpha = 1))
```
